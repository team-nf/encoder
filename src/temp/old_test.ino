...
sensor_data_t *_calibration_data_g;
...
void setup() {
	...
	sensor_data_t sensor_datas[3] = { {1,2,3}, {3,4,5}, {5,6,7} };
	...
	/* bu olaya el at */
	const calibration_data_t temp_data = { _example_meta_g, sensor_datas[0], sensor_datas[1], sensor_datas[2] };
	calibration_write(&temp_data);
}

void loop() {
	calibration_data_t *read_data = eeprom_read(0);
	if (_calibration_check_sensor_data(read_data->sensor_datas, _sensor_num_g)) {
